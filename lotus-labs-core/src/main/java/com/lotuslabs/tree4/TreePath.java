 ...
 *
 */
public class TreePath<K extends Serializable> implements Serializable {
	/**
	 * Serial UID
	 */
	private static final long serialVersionUID = -8002447451403593379L;

	/** Path representing the parent, null if lastPathComponent represents
	 * the root. */
	private TreePath<K> parentPath;

	/** Last path component. */
	private K lastPathComponent;
	 ...
	 *         {@code null}
	 */
	public TreePath(K lastPathComponent) {
		if(lastPathComponent == null)
	 ...
	 *         {@code null}
	 */
	protected TreePath(TreePath<K> parent, K lastPathComponent) {
		if(lastPathComponent == null)
			throw new IllegalArgumentException("path in TreePath must be non null.");
		parentPath = parent;
		this.lastPathComponent = lastPathComponent;
	 ...
	 */
	protected TreePath(K[] path, int length) {
	 ...
	 *         range of this path
	 */
	public K getPathComponent(int index) {
	...
	public boolean equals(Object o) {
		...
		if(o instanceof TreePath) {
			TreePath<K> oTreePath = (TreePath<K>)o;

			if(getPathCount() != oTreePath.getPathCount())
				return false;
			for(TreePath<K> path = this; path != null;
					path = path.getParentPath()) {
	 ...
	 */
	public boolean isDescendant(TreePath<K> aTreePath) {
		if(aTreePath == this)
			return true;
	 ...
	 */
	public TreePath<K> pathByAddingChild(K child) {
		if(child == null)
	 ...
	 */
	public TreePath<K> getParentPath() {
		return parentPath;
	}
